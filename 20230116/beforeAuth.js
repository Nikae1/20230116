...
function nextJoinStep(step){
	...
	switch(step){
		...
		case 3:
			...
			//	serverCallByFetchAjax(formData, 'GroupDupChk', 'post', 'messageController(true, jsonData.message)');
		...
		case 4:
			...
			//	group.storeInfoList[0].storeAddr = userData.children[1].value;
				...
				//	group.storeInfoList[0].storeAddrDetail = userData.children[2].value;
					...
					formData.append('storeInfoList[0].empList[0].empCode','000');
					formData.append('storeInfoList[0].empList[0].empLevCode','L1');
					
				//serverCallByXHRAjax(formData, 'MemberJoin', 'post', 'registration');
				serverCallByFetchAjax(formData, 'MemberJoin', 'post', 'registration'); 
					
					
				/*// 서버로 부터 넘겨 받은 json객체에 직원정보 삽입 
	group.storeInfoList[0].empList[0].empLevelCode = 'L1';
	...
	serverCallByFetchAjaxUsingJson(JSON.stringify(group), 'MemberJoin', 'post', 'registration'); */	
...
/* 회원가입이 완료된 후 페이지의 전환을 하기위해서 */ 
function registration(jsonData){
	group = jsonData;
	alert('group + store ready go!!!!');
	console.log(group);
...
function isStoreCode(){
	let formData = new FormData;
	formData.append('storeInfoList[0].storeCode', document.getElementsByName('storeCode')[0].value);
	serverCallByFetchAjax(formData, 'isStoreCode', 'post', 'storeCode');
}

function storeCode(jsonData){
	group = jsonData;
...
//	const inputZone = document.getElementById("inputZone");
	...
	const login = ["storeCode", "empCode", "empPin"];
	const login2 = ["storeInfoList[0].storeCode", "storeInfoList[0].empList[0].empCode", "storeInfoList[0].empList[0].empPin"];
	let isSubmit;
		
		
		console.log(document.getElementsByName("storeCode")[0].value);
		console.log(document.getElementsByName("empCode")[0].value);
		console.log(document.getElementsByName("empPin")[0].value);
		
	for(let idx=0; idx<3; idx++){
		
		isSubmit = lengthCheck(document.getElementsByName(login[idx])[0])
		if(!isSubmit)	{
			(document.getElementsByName(login[idx])[0]).value = "";
			(document.getElementsByName(login[idx])[0]).setAttribute("placeholder", message[idx]);
			(document.getElementsByName(login[idx])[0]).focus();
			break;
		}
		form.appendChild(createInputBox("hidden", login2[idx], document.getElementsByName(login[idx])[0].value, ""));
	}
	//		form.appendChild("storeCode", (document.getElementsByName("storeCode")[0].value));
	//		form.appendChild("empList[0].empCode", (document.getElementsByName("empCode")[0].value));
	//		form.appendChild("empList[0].empPin", (document.getElementsByName("empPin")[0].value));
	
	let publicIp = "106.243.194.229";
	
	console.log("start : access 1");
