...
public class Authentication extends TransactionAssistant {
	...
	private void systemAccessCtl(ModelAndView mav) {
		GroupBean group = (GroupBean)mav.getModel().get("GroupBean");
		String page = "index";
		String message = null;
		
		group.getStoreInfoList().get(0)
		.getEmpList().get(0)
		.getAccessList().get(0)
		.setAccessBrowser(this.clientInfo.getBrowserInfo(util.getHeaderInfo(false)));
		group.getStoreInfoList().get(0)
		.getEmpList().get(0)
		.getAccessList().get(0).setAccessLocation(util.getHeaderInfo(true));

		mav.addObject(group);
		
		try {
			if(this.enc.matches(group.getStoreInfoList().get(0).getEmpList().get(0).getEmpPin()
							, this.sqlSession.selectOne("getEmpPin", group))) {
